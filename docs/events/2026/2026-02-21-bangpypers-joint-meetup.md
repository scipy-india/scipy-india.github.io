---
date: 2026-02-21
day: Saturday
time: "11:00 AM – 2:00 PM IST"
location: "Amadeus Software Labs, Bengaluru"
status: past
feed: false
---

# BangPypers × SciPy India Meetup

A joint offline meetup with BangPypers — talks on LLM systems, NumPy contributions, Python t-strings, and more.

This page is a quick event reference. For the full write-up, use the recap link below.

::::{grid} 3
:gutter: 3

:::{grid-item-card} Date

21 February 2026
:::

:::{grid-item-card} Time

11:00 AM – 2:00 PM IST
:::

:::{grid-item-card} Venue

Amadeus Software Labs, Bengaluru
:::

::::

[Read the event recap](../../blog/2026-02-21-bangpypers-joint-meetup)
